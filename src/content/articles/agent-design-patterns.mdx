...
**Figure 1. Two loops can repeat for different reasons.** In the first, the agent selects its next action from new evidence, within a budget enforced by the system. In the second, a code-controlled workflow repeats a fixed sequence of agents until its exit condition is met.

<Mermaid
  label="Agent design pattern diagram"
  chart={`flowchart TB
  subgraph single["Single-agent tool loop"]
    direction TB
    E["Current evidence"] --> D{"Agent's next move"}
    D -->|Use tool| T["Call selected tool"]
    T --> O["Observe result"]
    O --> LIMIT{"Budget remains?"}
    LIMIT -->|Yes| E
    LIMIT -->|No| R["Return result or state uncertainty"]
    D -->|Done| R
  end

  subgraph fixed["Workflow-level loop"]
    direction TB
    C["Code controller"] --> A["Agent A: produce"]
    A --> B["Agent B: check"]
    B --> X{"Exit condition met?"}
    X -->|No, within limit| C
    X -->|Yes or cap reached| Y["Return result or escalate"]
  end`}
/>
...
**Figure 2. One possible composition for the support case.** The billing and shipping checks can be ordinary API calls or agents; the investigation and drafting are agent-led. A bounded checker loop reviews claims, and human approval applies only when the system proposes a refund.

<Mermaid
  label="Agent design pattern diagram"
  chart={`flowchart TB
  U["Support case: billing and delivery issue"] --> I["Fixed intake"]
  I --> P["Billing check: API or agent"]
  I --> S["Shipping check: API or agent"]
  P --> G["Gather evidence"]
  S --> G
  G --> A{"Agent has enough evidence?"}
  A -->|No, budget remains| T["Agent selects a read-only lookup"]
  T --> O["Tool returns observation"]
  O --> A
  A -->|Yes| D["Agent drafts answer"]
  A -->|No, budget spent| Q["Return qualified reply or escalate"]
  D --> C["Checker compares claims with records and policy"]
  C --> V{"Claims supported?"}
  V -->|No, revision budget remains| D
  V -->|No, cap reached| Q
  V -->|Yes| R{"Refund proposed?"}
  R -->|No| W["Send response; no refund issued"]
  R -->|Yes| H["Person reviews exact order and amount"]
  H -->|Approved| X["Issue refund"]
  H -->|Declined| W`}
/>
